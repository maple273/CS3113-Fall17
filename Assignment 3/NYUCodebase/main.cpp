...
void playerControl(GameState &game, float elapsed, SheetSprite playerSprite, SheetSprite bulletSprite, const Uint8 *keys) {
	//Player controls
		...
		case GAME_LEVEL:
			while (!done) {
				while (SDL_PollEvent(&event)) {
					...
					playerControl(game, elapsed, playerSprite, bulletSprite, keys);
